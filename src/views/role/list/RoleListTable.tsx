...
import UpdateRoleDrawer from '@/views/role/list/UpdateRoleDrawer'
...
// Actions
import { createRole, deleteRoleById, getRoleById, updateRole } from '@/libs/actions/roles'
import AssignRoleDialog from '@/views/role/list/AssignRoleDialog'
...
const RoleListTable = ({ tableData }: { tableData?: RoleType[] }) => {
  // States
  ...
  const handleOpenRoleDialog = (id: number) => {
    setSelectedId(id)
  ...
  const handleUpdate = async (val: RoleType, id: number) => {
    setIsLoading(true)

    try {
      const res = await updateRole({ name: val.name }, id)
  ...
  const handleOpenUpdateDrawer = async (id: number) => {
    const selectedData = await getRoleById(id)
  ...
  const handleCreate = async (val: RoleType) => {
    setIsLoading(true)

    try {
      const res = await createRole(val)
  ...
  const columns = useMemo<ColumnDef<RoleTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('teachersToRoles', {
        header: 'Guru',
        cell: ({ row }) => (
          <div className='flex items-center gap-2 flex-wrap divide-x'>
            {row.original.teachersToRoles.map(val => (
              <div key={val.teachers.id} className='p-2'>
                <p className='text-teal-600'>{val.teachers.name}</p>
              </div>
            ))}
          </div>
        )
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            ...
            <Tooltip title='Kelola Anggota'>
              <IconButton onClick={() => handleOpenRoleDialog(row.original.id)}>
  ...
  return (
    <>
      <Card>
        <CardHeader title='Data Role' className='pbe-4' />
      ...
      <UpdateRoleDrawer
        ...
        isLoading={isLoading}
        handleClose={() => setUpdateRoleOpen(!updateRoleOpen)}
      ...
      />
      <AssignRoleDialog open={roleDialogOpen} setOpen={setRoleDialogOpen} roleId={selectedId} />
    </>
  )
}
