...
type Props = {
  ...
  setData: (data: StudyYearType[]) => void
}
...
const AddStudyYearDrawer = (props: Props) => {
  // Props
  const { open, handleClose, studyYearData, setData } = props
  ...
  const onSubmit = (data: FormValidateType) => {
    const newStudyYear: StudyYearType = {
      id: (studyYearData?.length && studyYearData?.length + 1) || 1,

      name: data.name
    }

    setData([...(studyYearData ?? []), newStudyYear])
    handleClose()
    resetForm({ name: '' })
  }
