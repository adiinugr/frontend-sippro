...
const StudentInfo = ({ studentData }: StudentDataProps) => {
  const [lessonYearData, setLessonYearData] = useState<LessonYearType[]>([])
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setLessonYearData(lessonYearRes.result as LessonYearType[])
    }
      ...
      <CardContent className='flex flex-col pbs-12 gap-6'>
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex items-center justify-around flex-wrap gap-4'>
            {orderLessonYear(studentData.stTSbgTc).map(val => (
