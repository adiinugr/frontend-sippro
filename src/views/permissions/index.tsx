...
import { deletePermissionById } from '@/libs/actions/permissions'
...
const Permissions = ({ permissionsData }: { permissionsData?: PermissionRowType[] }) => {
  ...
  const handleDeleteData = async () => {
    ...
    try {
      const res = await deletePermissionById(selectedId)
  ...
  const columns = useMemo<ColumnDef<PermissionsTypeWithAction, any>[]>(
    () => [
        ...
        cell: ({ row }) =>
          typeof row.original.roles === 'string' ? (
            <Chip
              ...
              color={colors[row.original.roles]}
              size='small'
      ...
      columnHelper.accessor('createdDate', {
        header: 'Created Date',
        cell: ({ row }) => <Typography>{format(row.original.createdDate, 'dd MMMM yyyy')}</Typography>
      }),
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <Tooltip title='Edit'>
              <IconButton onClick={() => handleEditPermission(row.original.name, row.original.id)}>
  ...
  const table = useReactTable({
    data: permissionsData as PermissionRowType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      ...
      <PermissionDialog open={open} setOpen={setOpen} data={editValue} id={editId} />
      <DeleteDialog
        open={openDialog}
        isLoading={isLoading}
